...
После загрузки документа Orchestrator последовательно проходит этапы:

1. **Загрузка и дедупликация** — SHA-256, проверка дубликатов, ✦ запись в БД (status=uploaded), файл → MinIO
2. **OCR** — распознавание, чанкинг, извлечение классификации
3. **Валидация (RAG)** — embeddings, проверка структуры и кодов
4. **Approve** — подтверждение оператором (опционально)
5. **Promotion** — атомарная запись в Registry (nsi)
...
| **AI / NLP** | LangChain, OpenAI / отечественные LLM |
| **OCR-движки** | Docling, PaddleOCR, Tesseract |
