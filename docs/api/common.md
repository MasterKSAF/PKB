...
- Потоковая передача через SSE или WebSocket используется только для генерации ответа (UC‑04), остальные эндпоинты синхронны или возвращают идентификатор задачи для опроса статуса (сопоставление, обработка).
