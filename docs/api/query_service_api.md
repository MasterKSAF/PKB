...
Все ответы Query Service оборачиваются в единую структуру:
...
Обратная связь по ответу ассистента. Поддерживает два формата запроса: стандартный (с `session_id`/`message_id`) и UI-формат (с `answer_id`/`useful`).
