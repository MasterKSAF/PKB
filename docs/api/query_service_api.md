...
**Longpoll-механизм:** используется для ожидания завершения генерации ответа.

**Параметры запроса:**

| Параметр | Тип | По умолчанию | Описание |
|---|---|---|---|
| `longpoll` | int | `15` | Время ожидания в секундах. Сервер держит соединение, возвращая ответ при изменении статуса или по таймауту. |

**Возможные статусы ответа (`message.status`):**

| Статус | Описание |
|---|---|
| `pending` | Сообщение принято, ожидание обработки |
| `enriching` | Выполняется обогащение запроса через словарь терминов |
| `searching` | Выполняется поиск релевантных чанков (RAG Search) |
| `generating` | LLM генерирует ответ |
| `enriching_citations` | Выполняется обогащение цитирований |
| `answered` | Ответ готов и полностью сформирован |
| `failed` | Ошибка генерации ответа |

**Логика повторных запросов:** при получении нефинального статуса (`pending`, `enriching`, `searching`, `generating`, `enriching_citations`) клиент повторяет longpoll-запрос с тем же таймаутом до получения финального статуса (`answered` или `failed`).
