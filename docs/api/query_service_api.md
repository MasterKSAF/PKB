...
10. UI получает ответ при следующем опросе истории чата
...
UI использует этот endpoint для получения ответа. После отправки сообщения (статус `pending`) UI периодически опрашивает историю чата. Когда сообщение-ответ появится со статусом `answered`, результат готов к отображению.
...
Сообщение будет обработано асинхронно. UI переходит в состояние ожидания. Фактический результат (или запрос уточнения / конфликт) появится в истории чата при следующем опросе.
