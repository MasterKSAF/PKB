...
> **⏳ Требует реализации в коде**: необходима сетевая изоляция (Docker-сеть `internal`) для `/internal/*` эндпоинтов. Альтернатива — локальная JWT-верификация без вызова validate. Не входит в объём документации.
