...
### A36. Классификация ПКБ — `PKB_DOMAIN` как отдельная таблица `pkb_domains`
...
Поле `group` активно используется в API, но отсутствует в модели данных.

`PKB_DOMAIN` как `classifier_system` enum **не существует** — классификатор знает только `MKS`, `OKSTU`, `UDC`, `EXTERNAL`. Предметная область ПКБ не нормализована.

**Решение (13.06):** Вариант **B** — отдельная таблица `pkb_domains` с древовидной структурой, своя API-группа `/registry/pkb/*`.
- `PKB_DOMAIN` **не добавляется** в `classifier_system` enum — это разные домены (внешние стандарты vs внутренняя оргструктура)
- `classifier_code` — удалить из модели (deprecated, не используется в API)
- `industry_code` — удалить из модели (deprecated, не используется в API)
- `group` — объявить deprecated alias на `pkb_code`, который будет FK → `pkb_domains`

См. API `/registry/pkb/*` и модель 5.8 в `registry_service_api.md`.

Обоснование:
- Разные жизненные циклы: внешние классификаторы (MKS/OKSTU) — редко, внутренние (PKB) — часто
- Разные RBAC-модели управления
- Семантическая чистота: не смешивать внешние стандарты и оргструктуру предприятия
