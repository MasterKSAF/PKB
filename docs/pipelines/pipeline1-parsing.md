...
| Шаг | Действие | Результат |
|---|---|---|
| 3.1 | Сохранение карточки документа в `nsi_documents` (doc_code, title, order, validity_status) | `registry_doc_id`, ссылки на ресурсы |
...
**Выход:** тот же JSON, что и на входе, но с проставленными `id` для секций, `file_url` для изображений и блоком `registry` с `doc_id` и ссылками на ресурсы в БД. Эти данные используются RAG для построения чанков и цитирования.
