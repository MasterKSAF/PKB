...
Для сообщений, зависших в промежуточных состояниях, установлен единый таймаут ожидания — **48 часов** с момента входа в состояние. По истечении таймаута сообщение переводится в `failed` с кодом `MESSAGE_TIMEOUT`. Повторный запрос возможен только при явном повторном отправлении сообщения пользователем.

Таймаут отсчитывается с момента входа в состояние и проверяется Scheduler-сервисом.
