...
**Неполная индексация**: Если часть чанков сохранена в pgvector, а часть пропущена, документ индексируется частично. Для переиндексации используется `POST /documents/{doc_id}/reprocess`.

**Переиндексация**: Перед повторной индексацией (`POST /documents/{doc_id}/reprocess`) Оркестратор вызывает `DELETE /rag/build/{doc_id}` для очистки существующих чанков документа из векторного индекса. Только после успешного удаления запускается новый `POST /rag/build`. Если `DELETE` вернул ошибку, переиндексация отменяется с кодом `CLEANUP_FAILED`.
