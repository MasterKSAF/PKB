...
**Маршрутизация:** Gateway разделяет запросы к `/documents/*` и `/drafts/*` между Registry Service (чтение, CRUD) и Orchestrator Service (пайплайн, статус, управление). Подробнее — [gateway_service_api.md](api/gateway_service_api.md).
...
- Маршрутизация запросов к внутренним сервисам: чтение документов/черновиков → Registry, управление/пайплайн → Orchestrator
...
- Управление черновиками (drafts): создание (точка входа), preview, решение, жизненный цикл — вызов Registry internal API
...
- Вызов RAG Search для получения релевантных source-локаторов
- Формирование контекста и генерация ответа через LLM
- Обогащение цитирований machine-readable идентификаторами по правилу: **цитирование строится по `document_id + section_id`** (стабильные ID), `chunk_id` — технический, не用于 цитирования
...
**Назначение:**
Центральное хранилище всех данных: карточки документов, классификаторы (МКС, ОКСТУ, УДК), терминология, данные черновиков (drafts), категории. **Единый источник истины.** Orchestrator управляет черновиками через Registry internal API, чтение документов и черновиков — напрямую через Gateway. На этапе Формирования документа **пишет** данные в БД, на этапе Валидации **читает** справочники.
...
- **Вычисление бизнес-ключа** — **единственная точка** расчёта `title_hash_sha256` и `title_key` (формула нормализатора, терминологический реестр, детект аватаров)
...
- RAG хранит только актуальный индекс: перед новой индексацией старый полностью удаляется
- `chunk_id` — технический retrieval ID, **не用于 цитирования** (цитаты по `document_id + section_id`)
- `page` — 1-based, `bbox` — нормализованный 0..1
- `section_id` стабилен внутри документа

**Основные функции:**
- Приём плоского JSON с секциями от Registry (`parent_id`, `bbox`, `section_id` стабильный)
- **Чанкование** — разбиение секций на семантические фрагменты (до 1024 токенов) с учётом protected spans
- **Вычисление эмбеддингов** — векторные представления (Qwen3-Embedding-4B, 2048d, int8)
- **Построение векторного индекса** — сохранение чанков, эмбеддингов и индексов в pgvector
- Полное удаление старого индекса перед переиндексацией (`DELETE /rag/build/{doc_id}`)
- Longpoll-механизм с `indexing_txn_id` для отслеживания статуса
...
**Назначение:**
Поиск релевантных чанков по векторному индексу. Отвечает только за поиск и выдачу source-локаторов с retrieval-метаданными — без генерации ответа LLM. **Читает** данные из БД.

**Архитектурные принципы (RS-6, уточнение 20.06):**
- Все параметры поиска (`search_type`, `top_k`, `rerank`, `context_expansion`) — **только из app_settings**, не в API
- `chunk_id` — технический retrieval ID, `source` содержит стабильные идентификаторы для цитирования
- Ответ разделён на `source` (document_id, section_id, clause, path, page, bbox, content) и `retrieval` (chunk_id, score, mode)
- Context expansion — внутренний этап, возвращается в `context[]`

**Основные функции:**
- **Dense-поиск** (Qwen3-Embedding-4B, VECTOR(2048), cosine) → топ-кандидаты
- **Rerank** (bge-reranker-v2-m3-int8) через TEI-сервер
- **Context expansion** — добавление соседних чанков для каждого результата
- Фильтрация по `document_type`, `category_ids`, `document_ids`
- Возврат source-локаторов (для цитирования) и retrieval-метаданных (для отладки)
