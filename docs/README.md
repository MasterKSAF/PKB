...
│   ├── 23_05_26.md                   #   Расхождения документации (аудит)
│   ├── 26_05_26.md                   #   Принятые решения и открытые вопросы
│   ├── todo_26_05_29.md              #   TODO по итогам аудита
│   ├── migration_bigint_bbox_04_06_26.md  #   План перехода на bigint + bbox
│   ├── rag_experiments_methodology.md     #   Методика экспериментов RAG (индексация и поиск)
│   ├── rag_evaluation_methodology.md      #   Методика оценки качества и составления запросов
│   └── справочника предметных областей ПКБ.md
│
...
| Дата | Изменение |
|------|-----------|
| 04.06.2026 | **Методика экспериментов RAG**: полный перечень параметров, матрица запусков (3 фазы), метрики, псевдокод утилиты. См. `plans/rag_experiments_methodology.md`. |
| 04.06.2026 | **Переход на bigint**: все ID (`task_id`, `session_id`, `message_id`, `document_id`, `version_id`) — bigint (sequence). План: `plans/migration_bigint_bbox_04_06_26.md`. |
| 04.06.2026 | **bbox нормированный**: зафиксирован 0..1 для всех этапов (raw_ocr_v4 обновляется). Зафиксировано в `specificity.md`. |
| 04.06.2026 | Добавлены `specificity.md` (журнал аномалий) и `plans/` в структуру документации. |
| 29.05.2026 | **Аудит документации**: 19+10 задач закрыто, 6+5 осталось. См. `plans/todo_26_05_29.md`. |
| 26.05.2026 | **Принятые решения**: 20+ архитектурных решений зафиксированы. См. `plans/26_05_26.md`. |
