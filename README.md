...
3. Diagnostics server собирает данные через Python-модуль `diagnostics_server.py`

**Запуск diagnostics server на хосте:**
```bash
cd backend/diagnostics
python3 diagnostics_server.py 9090
```
...
# Полная сводка (диски, Docker, порты, логи)
...
curl http://localhost:8080/api/v1/system/diagnostics/parser

# Системные логи
curl http://localhost:8080/api/v1/system/diagnostics/system

# С указанием количества строк логов
curl "http://localhost:8080/api/v1/system/diagnostics?logs=50"
```

> **Замечание:** Если diagnostics server не запущен, Gateway вернёт собственную базовую диагностику (конфиг + health сервисов).
