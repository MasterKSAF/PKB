...
const ACCESS_TOKEN_KEY = 'pkb_gateway_access_token';
const REFRESH_TOKEN_KEY = 'pkb_gateway_refresh_token';
...
function mapGatewaySource(source: any, index = 0): Citation {
  return {
    ...
    document: source.document_title ?? source.document ?? source.document_id ?? 'Документ Gateway',
    ...
    version: source.version ?? 'Gateway',
...
function mapGatewayChatResponse(payload: any, query: string): ChatMessage {
  ...
  if (payload.scenario === 'needs_clarification') {
    return {
      ...
      role: 'assistant',
      content: `Gateway просит уточнить запрос: ${(payload.missing_fields ?? []).join(', ') || 'недостаточно контекста'}.`,
  ...
  if (payload.scenario === 'conflict') {
    return {
      id: payload.answer_id ?? payload.message_id ?? Math.random().toString(36).slice(2),
      role: 'assistant',
      content: payload.message ?? 'Gateway обнаружил конфликт источников.',
  ...
  const content =
    answerItems.length > 0
      ...
      : payload.content ?? payload.answer ?? payload.message ?? `Gateway принял запрос: ${query}`;
...
function mapGatewaySearchResponse(payload: any) {
  ...
  return items.map((item: any, index: number) => ({
    ...
    name: item.document_title ?? item.title ?? item.document_id ?? 'Документ Gateway',
    type: (item.document_type ?? item.source_type ?? item.type ?? 'PDF').toUpperCase(),
    version: item.version ?? 'Gateway',
    source: item.document_type ?? item.source ?? 'Gateway',
...
function mapGatewayHistoryResponse(payload: any): QueryHistoryItem[] {
  ...
  return items.map((item: any, index: number) => {
    ...
    return {
      ...
      user: item.user_name ?? item.user_id ?? 'Пользователь Gateway',
      project: item.project ?? item.project_name ?? 'Проект не указан',
      topic: item.topic ?? item.title ?? 'Чат Gateway',
...
function mapGatewaySessionsResponse(payload: any): QueryHistoryItem[] {
  ...
  return sessions.map((session: any, index: number) => {
    ...
    return {
      ...
      user: session.user_name ?? session.user_id ?? 'Пользователь Gateway',
      project: session.project ?? session.project_name ?? 'Диалоги Gateway',
      topic: session.topic ?? session.title ?? 'Диалог Gateway',
...
function mapGatewaySessionsToProjects(payload: any): GatewayChatProject[] {
  ...
  sessions.forEach((session: any, index: number) => {
    const projectName = session.project ?? session.project_name ?? session.workspace ?? 'Диалоги Gateway';
...
function mapGatewayMonitorLogs(payload: any): MonitorLogRow[] {
  ...
  if (!logs.length) {
    return [{ time: toUiTimestamp(), text: 'Gateway не вернул журнал проверки.', level: 'INFO' }];
  }

  return logs.map((row: any, index: number) => ({
    time: toUiTimestamp(row.time ?? row.timestamp),
    text: row.text ?? row.message ?? row.type ?? `Событие Gateway ${index + 1}`,
...
function mapGatewayQueueResponse(payload: any): ProcessingQueueItem[] {
  ...
  return queue.map((item: any, index: number) => ({
    ...
    document: item.title ?? item.document_title ?? item.document_id ?? 'Документ Gateway',
...
function mapGatewayUsersResponse(payload: any): AdminUser[] {
  ...
  return users.map((user: any, index: number) => {
    ...
    return {
      ...
      name: user.full_name ?? user.name ?? 'Пользователь Gateway',
...
function mapGatewayProfileToAdminUser(profile: any): AdminUser {
  ...
  return {
    ...
    name: profile.full_name ?? profile.name ?? profile.email ?? 'Пользователь Gateway',
...
function mapGatewayAuditResponse(payload: any): ProcessingLogItem[] {
  ...
  return events.slice(0, 20).map((event: any, index: number) => ({
    ...
    document: event.resource_id || event.resource_type || 'Gateway',
...
function backendUnavailableMessage(): ChatMessage {
  return {
    ...
    role: 'assistant',
    content: 'Gateway/backend недоступен. Повторите запрос позже или переключитесь в demo-режим.',
  ...
  send: async (query: string): Promise<ChatMessage> => {
    ...
    try {
      ...
      if (!activeSessionId) {
        ...
        activeSessionId = created.session_id ?? created.id;
        if (activeSessionId) {
          useUIStore.getState().setCurrentGatewaySessionId(activeSessionId);
        }
      }
      ...
      const response = await gatewayRequest<any>(() =>
        apiClient.post(`/chat/sessions/${activeSessionId}/messages`, {
          content: query,
        }),
      );
  ...
  query: async (q: string) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Gateway/backend недоступен');
      }
    }
  },
};
  ...
  dashboard: async (): Promise<MetricsDashboard> => {
    if (isDemoMode()) {
      return {
        ...
        logs: [{ time: toUiTimestamp(), text: 'Демонстрационный журнал QA.', level: 'INFO' }],
      };
    }
    ...
    try {
      ...
      return {
        control: MOCK_METRICS,
        answers: MOCK_ENGINEER_RATINGS,
        logs: [{ time: toUiTimestamp(), text: 'Gateway недоступен, показаны демонстрационные метрики.', level: 'WARN' }],
      };
    }
  },
};
