...
Если `3300` занят при Docker-запуске, нужно остановить другой контейнер или изменить порт в `docker-compose.yml`.

Если `3300` занят при dev-запуске, можно выбрать другой порт:

```bash
npm run dev -- --host 127.0.0.1 --port 3301 --strictPort
```
