...
- Вкладка `Проверка`: в Gateway пока нет отдельного контракта сверки проектных параметров с требованиями НСИ.
- `Мои проекты`: Gateway отдает chat-сессии, но не отдельную сущность проекта, поэтому проекты пока остаются UI-группировкой.
