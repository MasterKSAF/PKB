...
| `preview_metadata` | JSONB — **исходный JSON ответа Converter-validator preview** (`POST /converter/preview`). Содержит 14 полей: `doc_code`, `title`, `mks_oks_code`, `okstu_code`, `udk_code`, `pkb_codes`, `document_type`, `year`, `era`, `validity_status`, `issuing_body`, `jurisdiction`, `source_type`, `language`. **Без** `title_hash_sha256` и `title_key` — они вычисляются через `POST /validate/metadata` при необходимости (PATCH /metadata, PATCH /decide). Хранится целиком для истории и аудита. При approve копируется в `registry.documents.preview_snapshot`. |
...
| `preview_snapshot` | JSONB — исходный JSON ответа Converter-validator preview (`POST /converter/preview`), скопированный из `registry.drafts.preview_metadata` при approve. Содержит 14 полей (без `title_hash_sha256`/`title_key`). Хранится для истории и аудита. Не используется в поиске — только для просмотра исходных метаданных. |
