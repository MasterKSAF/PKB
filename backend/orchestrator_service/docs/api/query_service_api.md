...
| `sources[].section_id` | bigint | ID секции (стабильный, не chunk_id) |
| `sources[].clause` | string \| null | Номер пункта (напр. "4.2") |
| `sources[].path` | string | Путь секции (напр. "4/4.2") |
| `sources[].page` | int | Номер страницы (1-based) |
