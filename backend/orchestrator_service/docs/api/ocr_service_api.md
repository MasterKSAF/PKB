...
> **⚠️ АКТУАЛЬНО: Отдельный OCR-сервис не реализован.**
> Его роль выполняет Parser-сервис (`parser-service:8087`).
> Клиент `OCRServiceClient` в оркестраторе ходит на единый эндпоинт
> `POST /api/v1/parser/process` (не `/api/v1/ocr/process`).
> Документ ниже описывает **целевую архитектуру** — при появлении
> отдельного OCR-сервиса контракт останется тем же.
...
| Группа | Описание |
|--------|----------|
| `process` | Запуск OCR-распознавания, статус обработки, получение результата |

### Содержание

| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/ocr/process` | Запуск асинхронного OCR-распознавания (preview / full) |
| GET | `/ocr/process/{task_id}/status` | Статус обработки (longpoll) |
| GET | `/ocr/process/{task_id}/result` | Итоговый JSON-контейнер результата |
