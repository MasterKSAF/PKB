...
| Сервис | Порт |
|--------|------|
| **Nginx** (внешний веб-сервер) | `443` (HTTPS) |
| **Gateway** (внутренний) | `8080` |
| Orchestrator | `8081` |
| Auth | `8082` |
| Query | `8083` |
| Registry | `8084` |
...
> **Схема подключения:**
> ```
> Внешняя сеть → Nginx (:443) → Gateway (:8080) → Внутренние сервисы (:8081–8091)
> ```
> **Примечание:** В мок-режиме Gateway и Orchestrator объединены на порту `8081` (эмуляция nginx + gateway для разработки).
> Подробнее — [gateway_service_api.md](gateway_service_api.md).
