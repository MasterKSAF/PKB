...
| `failed` | Ошибка индексации (таймаут, превышение retry, нарушение целостности). Требуется переиндексация через Registry (`POST /api/v1/registry/documents/{doc_id}/reprocess`) |
...
**Неполная индексация**: Если часть чанков сохранена в pgvector, а часть пропущена, документ индексируется частично. Для переиндексации используется Registry (`POST /api/v1/registry/documents/{doc_id}/reprocess`).
...
- Документ становится кандидатом на переиндексацию через Registry (`POST /api/v1/registry/documents/{doc_id}/reprocess`, UI показывает предупреждение).
