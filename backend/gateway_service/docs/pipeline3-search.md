...
|---|---|---|
| `enriching` | 30 с | Пропустить обогащение, перейти к поиску без терминов |
| `searching` | 60 с | `failed` (RAG Search недоступен) |
| `generating` | 180 с (3 мин) | Retry 2 с усечением контекста; при неудаче — `failed` с `LLM_GENERATION_FAILED` |
