...
| ID раздела           | `section_id`       | Стабилен внутри документа                     |
...
| Заголовок раздела    | `section_title`    | Название раздела                              |
...
- Для UI источники возвращаются плоской структурой (поля `document_id`, `section_id`, `page`, `clause`, `path`, `excerpt`), без retrieval-метаданных (`chunk_id`, `score`, `mode`).
...
- `sources[]` — **плоская структура** (без retrieval-метаданных): `document_id`, `document_title`, `section_id`, `clause`, `path`, `page`, `excerpt`
- `chunk_id`, `mode`, `score` **не сохраняются** в истории — это технические метаданные поиска, не нужные для replay
