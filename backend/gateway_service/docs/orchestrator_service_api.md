...
Список черновиков. Поддерживает фильтрацию по бизнес-ключу документа, ID черновика или статусу.
Без параметров возвращает все черновики (для администратора).

**Query-параметры:**

| Параметр | Тип | Обязательный | Описание |
|----------|-----|-------------|----------|
| `document_key` | string | Нет | Бизнес-ключ документа (SHA-256). История попыток загрузить один документ |
| `draft_id` | bigint | Нет | ID конкретного черновика |
