...
| GET    | /registry/documents/{document_id}/file      | Получить файл последней/указанной версии | Реализовано |                      |
...
5. **Файлы и Версии:** При создании документа из черновика автоматически создаются записи в таблицах `files` и `document_versions` для связи документа с его исходным файлом.
6. **Превью страниц:** Эндпоинт превью страниц возвращает относительный путь в ключе `key` (например, `previews/{document_id}/p{page}.png`) вместо абсолютного URL с протоколом.
