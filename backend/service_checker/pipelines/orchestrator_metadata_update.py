#!/usr/bin/env python3
"""
PKB Neuroassistant — Pipeline: orchestrator_metadata_update

Обновление метаданных черновика через PATCH /drafts/{id}/metadata.

Проверяет OR-3b: обновление title, doc_code и других метаданных черновика.
"""
...
class OrchestratorMetadataUpdatePipeline(PipelineDef):
    """Пайплайн: обновление метаданных черновика."""
    ...
    def build_steps(self, context: PipelineContext) -> List[PipelineStep]:
        ...
        # ── Шаг 5: Обновление метаданных (OR-3b, через Gateway) ──────
        steps.append(PipelineStep(
            name="Обновление метаданных (через Gateway)",
            service="gateway",
            method="PATCH",
        ...
        # ── Шаг 6: Проверка обновлённых полей (через Gateway) ─────────
        steps.append(PipelineStep(
            name="Проверка обновлённых метаданных (через Gateway)",
            service="gateway",
            method="GET",
            path="/api/v1/drafts/{draft_id}",
            port=18080,
