...
Утилита для запуска, остановки, проверки работоспособности и тестирования API-эндпоинтов сервисов **PKB Neuroassistant**. Является вспомогательным инструментом (не production-сервисом), который используется для:

- Локальной разработки и отладки сервисов
- Интеграционного тестирования (Docker + реальные сервисы)
- Генерации отчётов о покрытии API
- Сбора логов supervisord

> 🔹 **Особенность:** service_checker НЕ вмешивается в работу других сервисов. Его задача — только проверить их состояние, выполнить тестовые API-вызовы и сформировать отчёт. Любые изменения конфигурации, данных или кода сервисов ЗАПРЕЩЕНЫ.
...
6. **TEI (Text Embeddings Inference)** — локальный сервер эмбеддингов, ONNX int8 модель `TrendHD/rubert-tiny2-int8` (312 dim), порт 8092, bind mount `./tei_model:/data`
