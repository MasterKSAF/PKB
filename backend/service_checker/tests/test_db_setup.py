...
SERVICES_REQUIRING_CREATE_ALL: dict[str, dict] = {
    "auth_service": {
        "path": "auth_service/app/main.py",
        "schema": "public",
        "reason": "Auth Service: users, roles, audit_log",
        "startup": [".create_all", "init_db"],
    },
    "query_service": {
        "path": "query_service/app/main.py",
        "schema": "public",
        "reason": "Query Service: sessions, messages",
        "startup": [".create_all", "init_db"],
    },
    ...
    # --- те, кто НЕ создаёт (ожидаем fail) ---
    "registry_service": {
    ...
    "rag_builder_service": {
        ...
        "schema": "rag",
        "reason": "RAG Builder: document_chunks (с HNSW/GIN индексами)",
        "startup": [".create_all"],
    },
    # --- consumer, НЕ должен создавать ---
    "rag_search_service": {
        "path": "rag_search_service/app/main.py",
        "schema": "rag",
        "reason": "RAG Search: consumer, читает document_chunks",
        "startup": [],  # не должен иметь create_all
    },
}
...
class TestServiceStartupCreateAll:
    """
    Проверка: какой сервис создаёт свои таблицы при старте.

    Читает файл startup каждого сервиса и ищет вызов create_all.
    Это статический анализ — без запуска сервиса.
    """

    @pytest.mark.parametrize("svc", [
        k for k, v in SERVICES_REQUIRING_CREATE_ALL.items()
        if v["startup"]  # только те, кто ДОЛЖЕН создавать
    ])
    def test_service_has_create_all(self, project_root, svc):
        """Проверка, что сервис вызывает create_all() при старте."""
        info = SERVICES_REQUIRING_CREATE_ALL[svc]
        ...
        # Ищем любой из маркеров startup
        markers = info["startup"]
        found = [m for m in markers if m in content]

        assert found, (
            f"❌ {svc} ({info['schema']}): НЕТ create_all() в {info['path']}\n"
            f"   Причина: {info['reason']}\n"
            f"   Искал маркеры: {markers}\n"
            f"   Пример (другие сервисы):\n"
            f"     auth_service:   @app.on_event('startup') → init_db()\n"
            f"     query_service:  lifespan → init_db()\n"
            f"     orchestrator:   lifespan → conn.run_sync(Base.metadata.create_all)\n"
            f"     integration:    Base.metadata.create_all(bind=engine) при импорте\n"
        )

    def test_rag_search_is_consumer(self, project_root):
        """RAG Search — consumer, не должен иметь create_all."""
        info = SERVICES_REQUIRING_CREATE_ALL["rag_search_service"]
        file_path = project_root / info["path"]
        ...
        content = file_path.read_text(encoding="utf-8")
        has_create_all = "create_all" in content

        assert not has_create_all, (
            f"RAG Search не должен создавать таблицы, "
            f"но найден create_all в {info['path']}"
        )

    def test_all_services_covered(self):
        """Проверка, что список покрывает все сервисы с БД."""
        expected = {
            "auth_service", "query_service", "orchestrator_service",
            "integration_service", "registry_service",
            "rag_builder_service", "rag_search_service",
        }
        actual = set(SERVICES_REQUIRING_CREATE_ALL.keys())
        missing = expected - actual
        extra = actual - expected

        assert not missing, f"Не охвачены сервисы: {missing}"
        assert not extra, f"Лишние в списке: {extra}"
