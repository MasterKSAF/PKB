...
## 50. source_draft_id убран из пайплайнов (2026-06-22)

### Аномалия
Во всех пайплайнах (`document_processing`, `full_document_lifecycle`, `multi_document_cross_search`, `services/registry.py`) передавался `source_draft_id: 1` — заглушка, без связи с реальным черновиком.

Таблицы `registry.drafts` не существует — черновики не реализованы. Передача заглушки создавала ложное впечатление корректной бизнес-логики.

### Исправление
- `source_draft_id: 1` убран из всех старых пайплайнов
- В новом `document_approval` `source_draft_id` передаётся как `{draft_id}` (плейсхолдер из шага создания черновика) с `skip_if` при сбое черновика
- Путь создания документа через черновик (POST /drafts → Registry) пока не работает
