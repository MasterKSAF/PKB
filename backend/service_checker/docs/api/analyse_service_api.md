...
| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/analyse/compare` | Сопоставление нормы и проектных данных (асинхронно) |
| GET | `/analyse/compare/{comparison_id}` | Статус и результат сравнения |
| POST | `/analyse/compare/batch` | Пакетное сравнение нескольких проектов |
| POST | `/analyse/calculate` | Арифметические вычисления |
| POST | `/analyse/recommend` | Рекомендации по исправлению ошибок |
...
Краткая выжимка:
- Внешние запросы проходят через Gateway (:18080).
