## API Converter-validator Service (converter-validator:18086)
...
**Базовый URL (внутренний)**: `http://127.0.0.1:18086/api/v1`
...
| Группа | Описание |
|--------|----------|
| `converter` | Конвертация сырого JSON в иерархический типизированный JSON (preview / full) |
| `validate` | Валидация структуры, метаданных и классификационных кодов |

### Содержание

| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/converter/preview` | Извлечение метаданных (preview, без бизнес-ключа) |
| POST | `/converter/convert` | Полная конвертация + валидация + LLM + кросс-ссылки |
| POST | `/validate/document` | Комплексная валидация документа без переконвертации |
| POST | `/validate/metadata` | Нормализация метаданных и вычисление бизнес-ключа |
