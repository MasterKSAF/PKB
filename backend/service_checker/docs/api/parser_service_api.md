## API Parser Service (parser-service:18087)
...
**Базовый URL (внутренний)**: `http://127.0.0.1:18087/api/v1`
...
| Группа | Описание |
|--------|----------|
| `process` | Запуск парсинга, статус обработки, получение результата |

### Содержание

| Метод | Путь | Описание |
|-------|------|----------|
| POST | `/parser/process` | Запуск асинхронного парсинга (preview / full) |
| GET | `/parser/process/{task_id}/status` | Статус обработки (longpoll) |
| GET | `/parser/process/{task_id}/result` | Итоговый JSON-контейнер результата |
