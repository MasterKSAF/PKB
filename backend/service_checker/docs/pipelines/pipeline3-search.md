...
**Вход:** массив источников от RAG Search (source-локаторы + retrieval-метаданные).

> **Архитектурное правило (RS-6, уточнение 20.06):** Цитирование строится по `document_id + section_id`. `chunk_id` — только технический retrieval ID, не用于 цитирования.

**Процесс:**

| Шаг | Действие | Результат |
|---|---|---|
| 3b.1 | Отбор top_k релевантных источников | Контекст для LLM |
| 3b.2 | Формирование промпта (вопрос + тексты источников с метаданными) | Промпт для генеративного движка |
...
**Промпт (шаг 3b.2):** В инструкцию LLM включается обязательное требование: после каждого абзаца указывать ссылку на источник в формате `[source:N]`, где N — **индекс элемента в массиве sources** (0-based), а не chunk_id. Пример: «Толщина обшивки должна быть не менее 12 мм [source:0].»

**Валидация (шаг 3b.4):**
- Из ответа LLM извлекаются все `[source:N]`
- Каждый N проверяется на вхождение в диапазон `[0, len(sources))` (индекс элемента массива sources)
...
**Вход:** текст ответа LLM + массив источников от RAG Search (source-локаторы).

**Процесс:**

| Шаг | Действие | Результат |
|---|---|---|
| 4.1 | Формирование строк-сносок с идентификаторами для каждого источника | `(источник: «Правила РС» §4.2 [document_id:42, section_id:420042], стр. 42)` |
...
1. Получить текстовый ответ от LLM (с `[source:N]`-маркерами).
2. Сопоставить `[source:N]` с массивом `sources` (по индексу N).
3. Для каждого источника сформировать machine-readable сноску по правилу:
   - Цитирование строится по `document_id + section_id` (стабильные идентификаторы).
   - `chunk_id` **не включается** в сноску — это технический retrieval ID.
4. Добавить сноски в конец ответа или заменить `[source:N]` на аннотированную версию.

**Пример:**

Исходный ответ модели:
> ... не менее **12 мм** [source:0].

После обработки Query Service:
> ... не менее **12 мм** (источник: «Правила РС» [document_id:42, section_id:420042]).
...
1. Все machine-readable идентификаторы (`document_id`, `section_id`) сверяются с массивом `sources`.
