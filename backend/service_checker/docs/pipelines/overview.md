...
| **Бизнес-ключ `title_hash_sha256`**            | Вычисляется как SHA-256 от `era | source_type | mks_oks_code | okstu_code | doc_code | normalized_title` — исключает коллизии (ГОСТ СССР vs ГОСТ РФ с одинаковым номером). Наряду с хешем сохраняется `title_key` — исходная строка конкатенации (см. `specifications/normalizer_specification.md` §2.1.1). Детальный алгоритм: `specifications/normalizer_specification.md` |
...
| **Rate Limiting для всех эндпоинтов через Gateway** | Единая политика ограничения запросов с разными лимитами для разных групп эндпоинтов. Nginx `limit_req` на Gateway. Код ошибки `429 TOO_MANY_REQUESTS`. |
