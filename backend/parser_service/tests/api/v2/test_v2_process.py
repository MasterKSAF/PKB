...
# ===================== PREVIEW TESTS =====================
def test_v2_preview_success(client, clear_task_store):
    """Успешный preview возвращает 202 (согласно декоратору) и структуру ResultResponse"""
    with patch("app.api.v2.endpoints.process.fetch_and_validate", new_callable=AsyncMock) as mock_fetch:
        ...
        with patch("app.services.file_loader.Validator.validate", new_callable=AsyncMock) as mock_validator:
            ...
            with patch("app.api.v2.endpoints.process.Pipeline.create") as mock_pipeline_factory:
...
def test_v2_preview_missing_max_pages(client):
...
def test_v2_preview_forbidden_options(client):
...
def test_v2_preview_extract_images_forbidden(client):
...
# ===================== FULL MODE TESTS =====================
def test_v2_full_accepted(client, clear_task_store):
...
def test_v2_full_idempotent(client, clear_task_store):
