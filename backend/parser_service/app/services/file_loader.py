...
async def fetch_and_validate(file_key: str) -> bytes:
    ...
    try:
        ...
        logger.info("File downloaded successfully: %s, size=%d bytes", file_key, len(file_bytes))
    ...
    try:
        mime = await Validator.validate(file_bytes, original_filename)
        logger.debug("Validation passed for %s, MIME=%s", file_key, mime)
    except Exception as e:
        logger.error("Validation failed for %s: %s", file_key, str(e), exc_info=True)
        raise
