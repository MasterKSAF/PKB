...
class IndexingService:
    ...
    async def delete(self, document_id: int, session: AsyncSession) -> DeleteResponse:
        ...
        try:
            repo = ChunkRepository(session)
            async with session.begin():
                deleted = await repo.delete_by_document(document_id)
    ...
    async def status(self, document_id: int, session: AsyncSession, longpoll: int) -> StatusResponse:
        doc_id = str(document_id)
        logger.debug("Status check start document_id={} longpoll={}", doc_id, longpoll)
        try:
            if doc_id not in self._status:
                repo = ChunkRepository(session)
                count = await repo.count_by_document(document_id)
                has_emb = await repo.has_embeddings(document_id)
                self._status[doc_id] = DocStatus(
            ...
            return StatusResponse(
                ...
                status=cur.status,
                chunks_count=cur.chunks_count,
                has_embeddings=cur.has_embeddings,
