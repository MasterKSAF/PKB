...
### П7a. diagnostics может быть медленным — ждать до 2 мин

`GET /api/v1/system/diagnostics?verbose=true` последовательно опрашивает все сервисы через docker exec и HTTP.
Даже с агрессивными таймаутами (5-10с) **некоторые проверки могут ждать полный таймаут** — например,
если postgres перегружен или infinity не отвечает.

**Норма:** compact diagnostics ~10-20с, verbose ~30-60с. При проблемах с docker/сетью — до 120с.
Не прерывать запрос раньше 120с — иначе получите Transport error вместо данных.

Если diagnostics стабильно падает по таймауту (>120с) — значит какой-то subprocess висит бесконечно.
В этом случае идти напрямую: `GET /api/v1/system/logs/{service}?lines=200` — этот endpoint просто
читает docker логи без сложных проверок.
